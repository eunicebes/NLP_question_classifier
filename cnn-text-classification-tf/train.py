...
tf.flags.DEFINE_string("question_file", "../questions_nondup.csv", "Questions without duplications")
...
# vocab_dict = dict(zip(vocab_dict.values(), vocab_dict.keys()))
# x_3 = np.zeros((x.shape[0], x.shape[1], 2))
# VA_dict = wordProcBase.get_VA_word_dict()
# for i in range(x.shape[0]):
#     for j in range(x.shape[1]):
#         if vocab_dict[x[i,j]] in VA_dict.keys():
#             x_3[i,j,0] = VA_dict.get(vocab_dict[x[i,j]], 0)['V_value']
#             x_3[i,j,1] = VA_dict.get(vocab_dict[x[i,j]], 0)['A_value']
#         else:
#             x_3[i,j,0] = 0
#             x_3[i,j,0] = 0
# sys.exit()
